...
__global__ void DCT_filter(int *image, int* image_width, int *pixel_delay, double* DCT_Creator, double* DCT_Creator_T, int* window_size, double *threshold, int *result)
{
    int x = (blockIdx.x)* (*window_size) * (*pixel_delay) + (threadIdx.x);
    int y = (blockIdx.y)* (*window_size);
    printf("threadIdx: (%i); blockIdx: (%i %i); px = %i, py = %i\n", threadIdx.x, blockIdx.x, blockIdx.y, x, y);

    //double* image_block = new double[(*window_size)*(*window_size)];
    int p_dy = y;
    int p_dx = x;
    /*for (int k = 0; k < (*window_size); k++, p_dy++)
        for (int t = 0; t < (*window_size); t++, p_dx += (*pixel_delay)) {
...
int* DCT_Filrer(int *image_, int width_, int heigth_, int channels_)
{
    int wind_size = 8;
    int image_width = width_;
    double threshold = 255*0.012;
    int *result = new int[image_width*heigth_];

    double DCT_Creator8[8][8] = {
    ...
    cudaMalloc((void**)&image_dev, sizeof(int)*image_width*heigth_); // 256 is image size
    ...
    cudaMalloc((void**)&result_dev, sizeof(int)*image_width*heigth_);


    unsigned int start_time = clock();
    cudaMemcpyAsync(image_dev, image_, sizeof(int)*image_width*heigth_, cudaMemcpyHostToDevice);   //copy image to videomemory
    ...
    switch(wind_size){   // copy DCT creator to video
        case 8:
            cudaMemcpyAsync(DCT_creator_dev, DCT_Creator8, sizeof(double)*wind_size*wind_size, cudaMemcpyHostToDevice);
            cudaMemcpyAsync(DCT_creator_T_dev, DCT_Creator8_T, sizeof(double)*wind_size*wind_size, cudaMemcpyHostToDevice);
    }
    ...
    cudaMemcpyAsync(result, result_dev, image_width * heigth_ * sizeof(int), cudaMemcpyDeviceToHost);
